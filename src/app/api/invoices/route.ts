...
export async function GET(request: NextRequest) {
  try {
    ...
    const autoGenerated = searchParams.get('autoGenerated') // 'auto', 'manual'
    ...
    let filtered: FilteredInvoice[] = allInvoices
      .map(invoice => {
        ...
        return {
          ...
          autoGenerated: (invoice as any).autoGenerated,
          linearTaskCount: (invoice as any).linearTaskCount,
          linearTasks: (invoice as any).linearTasks,
        } as FilteredInvoice
      })
    ...
    if (autoGenerated) {
      if (autoGenerated === 'auto') {
        filtered = filtered.filter(inv => inv.autoGenerated === true)
      } else if (autoGenerated === 'manual') {
        filtered = filtered.filter(inv => inv.autoGenerated !== true)
      }
    }
